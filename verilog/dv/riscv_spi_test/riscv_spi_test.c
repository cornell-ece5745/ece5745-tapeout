 ...
 */
 ...
 void spi_write(char c, int length)
{
    uint32_t control_value = (length << 8)+1;
...
//    reg_spimaster_control = 0x0800;
    reg_spimaster_control = control_value;
...
void main()
{
    ...
    // reg_mprj_io_1  = GPIO_MODE_MGMT_STD_BIDIRECTIONAL;	// SDO
    // reg_mprj_io_3  = GPIO_MODE_MGMT_STD_OUTPUT;	// CSB
    // reg_mprj_io_4  = GPIO_MODE_MGMT_STD_OUTPUT;	// SCK
    // reg_mprj_io_0  = GPIO_MODE_MGMT_STD_OUTPUT;	// CSB
    ...
    // Reset
    reg_spimaster_cs = 0x0001;  // sel=0, manual CS
    spi_write(0xff, 8);
    reg_spimaster_cs = 0x0000;  // release CS
    reg_spimaster_cs = 0x0001;
    spi_write(0xab, 8); // wakeup
    reg_spimaster_cs = 0x0000;  // release CS
    ...
    spi_write(0x03, 8);        // Write 0x03 (read mode)
    spi_write(0x00, 8);        // Write 0x00 (start address high byte)
    spi_write(0x00, 8);        // Write 0x00 (start address middle byte)
    spi_write(0x04, 8);        // Write 0x04 (start address low byte)

    value = spi_read();
    reg_mprj_datal = (value << 8);
    ...
    spi_write(0x00, 8);        // Write 0x00 (start address high byte)
    spi_write(0x00, 8);        // Write 0x00 (start address middle byte)
    spi_write(0x08, 8);        // Write 0x08 (start address low byte)
    ...
    spi_write(0x00, 8);        // Write 0x00 (start address high byte)
    spi_write(0x00, 8);        // Write 0x00 (start address middle byte)
    spi_write(0x0a, 8);        // Write 0x0a (start address low byte)
    ...
    reg_spimaster_cs = 0x0000;  // release CS
    reg_spimaster_cs = 0x10004;  // sel=0, manual CS
