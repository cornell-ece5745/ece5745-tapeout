...
class SPITestHarness( object ):
  ...
  def __init__( s, DESIGN, num_components, spi_bits, cmdline_opts, trace=True ):
    ...
    s.dut.spi_min__cs @= 1
    s.dut.spi_min__sclk @= 0
    s.dut.spi_min__mosi @= 0
    s.dut.spi_min__miso @= 0
  ...
  def _start_transaction( s ): #send bits
    ...
    for i in range(3): # cs = 1
      ...
      s.dut.spi_min__sclk        @= 0
      s.dut.spi_min__cs          @= 1
      s.dut.spi_min__mosi        @= 1 # mosi is a dont care here bc CS is high
  ...
  def _send_bit( s, mosi): #send bits
    ...
    for i in range(3): # sclk = 0
      ...
      s.dut.spi_min__sclk        @= 0
      s.dut.spi_min__cs          @= 0
      s.dut.spi_min__mosi        @= mosi
      s.dut.sim_eval_combinational()
      # Tick simulator one cycle
      s.dut.sim_tick()

    for i in range(3): # sclk = 1
      s.dut.spi_min__sclk        @= 1
      s.dut.spi_min__cs          @= 0
      s.dut.spi_min__mosi        @= mosi
      s.dut.sim_eval_combinational()
      s.dut.sim_tick()
    ...
    return copy.deepcopy(s.dut.spi_min__miso) 