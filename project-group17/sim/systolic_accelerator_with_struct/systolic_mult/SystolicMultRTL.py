...
class SystolicMultVRTL( VerilogPlaceholder, Component ):
  ...
  def construct( s ):
    ...
    s.recv = stream.ifcs.RecvIfcRTL(SystolicMsgs.recv)
    s.send = stream.ifcs.SendIfcRTL(SystolicMsgs.send)

    # If translated into Verilog, we use the explicit name

    s.set_metadata( VerilogTranslationPass.explicit_module_name, 'SystolicMultRTL' )

# Import the appropriate version based on the rtl_language variable

if rtl_language == 'verilog':
  SystolicMultRTL = SystolicMultVRTL
else:
