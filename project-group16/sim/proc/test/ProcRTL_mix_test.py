...
def test_jal_beq( name, test, cmdline_opts ):
  run_test( ProcRTL, test, cmdline_opts=cmdline_opts )

def test_jal_beq_rand_delays():
  run_test( ProcRTL, inst_jal_beq.gen_basic_test,
...
def test_mul_mem( name, test, cmdline_opts ):
  run_test( ProcRTL, test, cmdline_opts=cmdline_opts )


def test_mul_mem_rand_delays():
  run_test( ProcRTL, inst_mul_mem.gen_more_test,
            src_delay=3, sink_delay=5, mem_stall_prob=0.5, mem_latency=3 )
