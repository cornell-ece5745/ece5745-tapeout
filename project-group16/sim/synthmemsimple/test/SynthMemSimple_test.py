...
class TestHarness( Component ):
  ...
  def construct( s, dut ):
    ...
    s.src.send  //= s.regfile.minion.req
    s.sink.recv //= s.regfile.minion.resp

  def done( s ):
    return s.src.done() and s.sink.done()

  def line_trace( s ):
    return f"{s.src.line_trace()} > ({s.regfile.line_trace()}) > {s.sink.line_trace()}"
...
def test( test_params, cmdline_opts ):
  ...
  top = TestHarness( SynthMemSimple() )
  ...
  run_sim( top, cmdline_opts, duts=['regfile'] )
